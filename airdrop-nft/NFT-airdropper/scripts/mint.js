...
const existingContractAddr = "0x276646040FF78B339F7D51117348Cb454f0261FB";

async function main() {
  const nft = await hre.ethers.getContractAt("AirdropperNFT", existingContractAddr);

  const signer0 = await ethers.provider.getSigner(0);
  const nonce = await signer0.getTransactionCount();
  for(let i = 0; i < friends.length; i++) {
    const tokenURI = "https://gateway.ipfs.io/ipfs/QmUtRoZPqBZUYxJvS5wSeT1wYMEWTMMacvHocsRyRA9yc7";
